...
func TestDefaultApplicationEnvHandler_ServeHTTP(t *testing.T) {
	t.Run("Should work without error (happytest)", func(t *testing.T) {
		someKeyToMaskExplicitly := "someKeyToMaskExplicitly"
		someValueToMaskExplicitly := "someValueToMaskExplicitly"

		sktme, sktmepexists := os.LookupEnv(someKeyToMaskExplicitly)
		os.Setenv(someKeyToMaskExplicitly, someValueToMaskExplicitly)

		request, _ := http.NewRequest("GET", "http://localhost:8081/env", nil)
		response := httptest.NewRecorder()
		appEnvHandler := DefaultApplicationEnvHandler()
		appEnvHandler.SetKeysToMask([]string{someKeyToMaskExplicitly})
		...
		assert.Contains(t, response.Body.String(), "systemEnvironment")
		assert.Contains(t, response.Body.String(), "***")
		assert.NotContains(t, response.Body.String(), someValueToMaskExplicitly)

		if sktmepexists {
			os.Setenv(someKeyToMaskExplicitly, sktme)
		} else {
